...
def crop_face_from_video(video_path,cache_path,crop_path,clip_size, clip_limit=280):
    # mp4 파일이 아니면 return
...
if __name__ == '__main__':
    ...
    p.add_argument('--save_path','-s', type=str, default='/workspace/datasets/DF40')
    ...
    for method in Extract_dataset.keys():
        print(f"start {method} dataset preprocessing")
        splits = ['train','test']
        for split in splits:
            if split=='train':
                data_path = Extract_dataset[method][0]
            else:
                data_path = Extract_dataset[method][1]
            print(f"data {split} path : {data_path}")
            
            videos_paths = glob.glob(join(data_path,'*.mp4'), recursive=True)
            if len(videos_paths)==0:
                print(f"there is no video in {data_path}")
                continue
            print(f"number of videos : {len(videos_paths)}")
            
            for video_path in tqdm(videos_paths):
                video_name = video_path.split('/')[-1].replace('.mp4','')
                crop_path = join(save_path, method, split, video_name)
                if not os.path.exists(crop_path):
                    os.makedirs(crop_path)
                cache_file_path = join(cache_path, method, split)
                if not os.path.exists(cache_file_path):
                    os.makedirs(cache_file_path)
                crop_face_from_video(video_path, join(cache_file_path, video_name), crop_path, clip_size, clip_limit= 280 if split=='train' else 120)
